...
/*** 如接入方用统计功能需要进行初始化(最好在初始化ShareInstall SDK之前调用)
* @ param apptypeid App软件唯一识别符(接入方配置，若没有可填appkey)
* @ param appcqid 接入方自己配置的渠道号 如：appstore,若没有可传空字符串
* @ param appqid 接入方配置的渠道号后面+日期
* @ param accid APP注册ID
* @ param muid  平台注册ID
* @ param istourist 是否是游客，如：1表示游客、0表示非游客
**/

+(void)apptypeid:(NSString *)apptypeid appcqid:(NSString *)appcqid appqid:(NSString *)appqid accid:(NSString *)accid muid:(NSString *)muid istourist:(NSString *)istourist;
