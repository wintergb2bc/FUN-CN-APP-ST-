						...
						{LiveInsightsSocketData && LiveInsightsSocketData != '' ? (
							LiveInsightsSocketData.map((item, key) => {
								...
								return (
									...
									// 		{T && <Text style={{ color: '#999999', fontSize: 12}}>{item.data.minutes}'</Text>}
									// 		{T && <SportImage imgsID={HomeTeamId} imgSize={14} Vendor={Vendor} />}
									// 		<Text style={{ color: '#999999', fontSize: 12, paddingLeft: 5, paddingRight: 5 }}>{T ? HomeTeamName : AwayTeamName}</Text>
									// 		{!T && <SportImage imgsID={AwayTeamId} imgSize={14} Vendor={Vendor} />}
									// 		{!T && <Text style={{ color: '#999999', fontSize: 12}}>{item.data.minutes}'</Text>}
									// 	</View>
									// 	<View style={styles.message}>
									// 		{T && <View style={styles.leftIcon} />}
									// 		<Text style={{ padding: 10, fontSize: 12, lineHeight: 19 }}>{Message}</Text>
