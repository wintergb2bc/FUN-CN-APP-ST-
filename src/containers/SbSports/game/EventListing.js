		...
		fetchRequest(ApiPort.MiniGamesActiveGame, "GET")
			.then(res => {
				if (res.isSuccess && res.result) {
					...
					const now = moment().utcOffset(8).unix();
					const from = moment(result.eventStartDate).unix();
					const to = moment(result.eventEndDate).unix();

					// console.log(moment().utcOffset(8).format('YYYY/MM/DD HH:mm:ss'))
					// console.log(moment(result.eventStartDate).format('YYYY/MM/DD HH:mm:ss'))
					// console.log(moment(result.eventEndDate).format('YYYY/MM/DD HH:mm:ss'))
					
					if(now >= from && now <= to) {
						// 活動進行中
						window.sbWorldCupBtn = true;
						this.setState({ worldCup_open: true },);
					} else {
						window.sbWorldCupBtn = false;
						this.setState({ worldCup_open: false },);
					}
				}
			})
			.catch(err => {
		...
		return (
			<View style={{ flex: 1 }} onLayout={(e) => this.getGuide(e)}>
				...
				{worldCup_open && (
					<IconDrag
						useLocalSrc={true}
						OpenMenu={() => { this.setState({worldCup_open: false}) }}
