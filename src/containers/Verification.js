...
class SetVerification extends React.Component {
  ...
  getVerifyTimes() {
    ...
    fetchRequest(
      ...
      .then((res) => {
        Toast.hide();
        if (res.result) {
          this.setState({ verifyTimes: res.result.attempt });
  ...
  sendEmailVerfication() {
    ...
    fetchRequest(window.ApiPort.EmailVerify, "POST", data)
      ...
      .catch((err) => {
        // errCodeHandle(err ,"邮件服务异常，请稍后再试");
      });
  }
  //TODO: 校验短信验证码
  verifySMSTAC() {
                      ...
                      {getCodeEmail && (
                        <Touch
                          onPress={() => {
                            this.getCode();
                            if(codeAgainEmail){
                              if(this.props.from === "profile" || this.props.from === "user") {
                                PiwikAction.SendPiwik("ResendCode_Email_ProfilePage");
                                return;
                              }
                              PiwikAction.SendPiwik("ResendCodeEmail")
                            }else{
                              if(this.props.from === "profile" || this.props.from === "user") {
                                PiwikAction.SendPiwik("SendCode_Email_ProfilePage");
                                return;
                              }
                      ...
                      {getCodePhone && (
                        <Touch
                          onPress={() => {
                            ...
                            if(codeAgainPhone){
                              if(this.props.from === "profile" || this.props.from === "user") {
                                PiwikAction.SendPiwik("ResendCode_Phone_ProfilePage");
                                return;
                              }
                              PiwikAction.SendPiwik("ResendCodePhone")
                            }else{
                              if(this.props.from === "profile" || this.props.from === "user") {
                                PiwikAction.SendPiwik("SendCode_Phone_ProfilePage");
                                return;
                              }
                        ...
                        {verificaType == "email" && (
                          <Touch
                            ...
                            onPress={() => {
                              this.submitBtn();
                              if(this.props.from === "profile" || this.props.from === "user") {
                                PiwikAction.SendPiwik("Verify_Email_ProfilePage");
                                return;
                              }
                        ...
                        {verificaType == "phone" && (
                          <Touch
                            ...
                            onPress={() => {
                              this.submitBtn();
                              if(this.props.from === "profile" || this.props.from === "user") {
                                PiwikAction.SendPiwik("Verify_Phone_ProfilePage");
                                return;
                              }
